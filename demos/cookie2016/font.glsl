...
//#define line1 H_ e_ l_ l_ o_ _  S_ h_ a_ d_ e_ r_ t_ o_ y_  _ t_ h_ i_ s_ _ i_ s_ _ m_ y_
//#define line2 BOLD BLUE p_ r_ o_ p_ o_ r_ t_ i_ o_ n_ a_ l_ BLUE _ f_ o_ n_ t_ _dot _ I_ _ h_ o_ p_ e_ _ y_ o_ u_
...
//#define line6 _1 _2 _3 _4 _5 _6 _7 _8 _9 _0 _dot _comma _exclam _question _open1 _close1 _dotdot 
...
void main()
{
    ...
    float weight  = 0.03 / sequence_time;//0.08*vel+0.01+us.w*min(sequence_time*.02-.03,0.03);
		vec3 back_color = vec3(0.0);
		vec3 text_color = vec3(0.7, 0.7, 1.0);
	
		vec3 color = mix(text_color,back_color,smoothstep(weight-px,weight+px, x));
		vec3 color_inv = text_color-color;
	 	//color = pow(color, vec3(1./2.2));
		//color_inv = pow(color_inv, vec3(1./2.2));
		color = mix(color, color_inv,min(vel,abs(0.5*nuv.y))) / sequence_time;		
