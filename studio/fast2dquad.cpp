...
#define BUFFER_OFFSET(a) (static_cast<char*>(nullptr) + (a))
...
void Fast2DQuad::initializeGL(RenderFunctionsCache& gl)
{
	...
	vao.bind();
		...
		vbo.bind();
			...
			gl.glVertexAttribPointer(static_cast<GLuint>(VertexAttributesIndex::position), 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));
			gl.glEnableVertexAttribArray(static_cast<GLuint>(VertexAttributesIndex::position));
			gl.glVertexAttribPointer(static_cast<GLuint>(VertexAttributesIndex::texCoord), 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(4 * 2 * sizeof(float)));
			gl.glEnableVertexAttribArray(static_cast<GLuint>(VertexAttributesIndex::texCoord));
