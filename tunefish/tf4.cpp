...
void eTfSignalToPeak(eF32 **sig, eF32 *peak_left, eF32 *peak_right, eU32 length)
{
    ...
    eF32x2 peak = eSimdSetAll(0.0f);
		eF32x2 div = eSimdSetAll(static_cast<eF32>(length));

    while(length--)
    {
        eF32 left = *srcLeft++;
        eF32 right = *srcRight++;
				eU32 left_i = *(reinterpret_cast<eU32*>(&left));
				eU32 right_i = *(reinterpret_cast<eU32*>(&right));
        left_i &= 0x7fffffff;
        right_i &= 0x7fffffff;
				left = *(reinterpret_cast<eF32*>(&left_i));
				right = *(reinterpret_cast<eF32*>(&right_i));
...
eF32 eTfLfoProcess(eTfSynth &synth, eTfInstrument &instr, eTfLfo &lfoState, eU32 paramOffset, eU32 frameSize)
{
    ...
    eF32 depth = instr.params[paramOffset+1];
		eU32 shape = static_cast<eU32>(eFtoL(eRoundNearest(instr.params[paramOffset + 2] * (TF_LFOSHAPECOUNT - 1))));
...
eBool eTfModMatrixProcess(eTfSynth &synth, eTfInstrument &instr, eTfModMatrix &state, eU32 frameSize)
{
    ...
    for(eU32 i=0;i<TF_MODMATRIXENTRIES;i++)
    {
        ...
        else
        {
            ...
            mod = 1.0f + mod * (TF_MM_MODRANGE-1.0f);
				...
				state.entries[i].src   = static_cast<eTfModMatrix::Input>(eFtoL(eRoundNearest(instr.params[TF_MM1_SOURCE + i*3] * (eTfModMatrix::INPUT_COUNT-1))));
				state.entries[i].dst   = static_cast<eTfModMatrix::Output>(eFtoL(eRoundNearest(instr.params[TF_MM1_TARGET + i*3] * (eTfModMatrix::OUTPUT_COUNT-1))));
    ...
    for(eU32 i=0;i<TF_MODMATRIXENTRIES;i++)
    {
				state.modulation[i] = eTfModMatrixGet(state, static_cast<eTfModMatrix::Output>(eTfModMatrix::OUTPUT_MOD1 + i));
    }
...
{
		eInt sign = static_cast<eInt>(type);
    eF32 wr, wi, arg, *p1, *p2, temp;
    eF32 tr, ti, ur, ui, *p1r, *p1i, *p2r, *p2i;
    eInt i, bitm, j, le, le2, k;
		eF32 fsign = static_cast<eF32>(sign);
    eF32 sine, cosine;
		eInt count = eFtoL(eLog10(static_cast<eF32>(frameSize))/eLog10(2.0f));

		for (i = 2; i < static_cast<eInt>(2*frameSize-2); i += 2)
    {
				for (bitm = 2, j = 0; bitm < static_cast<eInt>(2*frameSize); bitm <<= 1)
        {
            if (i & bitm) j++;
            j <<= 1;
        }
    ...
    for (k = 0, le = 2; k < count; k++)
    {
        ...
        for (j = 0; j < le2; j += 2)
        {
            ...
            p2r = p1r+le2; p2i = p2r+1;
						...
						for (i = j; i < static_cast<eInt>(2*frameSize); i += le)
            {
                tr = *p2r * ur - *p2i * ui;
                ti = *p2r * ui + *p2i * ur;
...
void eTfGeneratorUpdate(eTfSynth &synth, eTfInstrument &instr, eTfVoice &voice, eTfGenerator &generator, eF32 frequencyRange)
{
    ...
    eU32 frameSizeHalf = frameSize / 2;
		eU32 genFrameSize = static_cast<eU32>(eFtoL(eF32(frameSizeHalf) * frequencyRange));
    ...
    scale       = eClamp<eF32>(0.0f, scale, 4.0f);
		...
		eU32 numHarmonics = 1+eMin(static_cast<eU32>(eFtoL(harmonics * TF_MAX_HARMONICS)), TF_MAX_HARMONICS);
        ...
        !eIsFloatZero(generator.activeBandwidth - bandwidth))
    {
        ...
        for (eU32 harmonicIndex=1; harmonicIndex < numHarmonics + 1; harmonicIndex++)
        {
            ...
            harmonicBandwidth[harmonicIndex-1] = 0.3f + (bandwidth * harmonicIndex);
						harmonicVolume[harmonicIndex-1] = 1.0f / ePow(static_cast<eF32>(harmonicIndex), 1.0f + damp);
        }
        ...
        for (eU32 i=0; i<genFrameSize; i++)
        {
            ...
            {
								eF32 dist = eAbs(*offsetPtr - static_cast<eF32>(i));
                dist /= *bandwidthPtr;

                if (dist < 5.0f)
                {
										eU32 expLookup = static_cast<eU32>(eFtoL(dist / 5.0f * (TF_MAXFRAMESIZE-1)));
    ...
    for (eU32 i=0; i<frameSizeHalf; i++)
    {
        ...
        if (randomizationOn)
            sineOffset = (generator.modulation * TF_FRAMESIZE) * (1.0f - (random * *randPtr++));
				...
				eU32 sinLookup = static_cast<eU32>(eFtoL(sineOffset * modulationStrength)) % TF_FRAMESIZE;
...
eBool eTfGeneratorProcess(eTfSynth &synth, eTfInstrument &instr, eTfVoice &voice, eTfGenerator &generator, eF32 velocity, eF32 **signal, eU32 frameSize)
{
    ...
    if (maxFrameVolume > 0.0f)
    {
        ...
        eF32 spread     = instr.params[TF_GEN_SPREAD];
				eU32 unisono    = static_cast<eU32>(eFtoL(eRoundNearest(instr.params[TF_GEN_UNISONO] * (TF_MAXUNISONO-1))) + 1);
        ...
        // -------------------------------------------------
				eU32 ioctave = static_cast<eU32>(eFtoL(eRoundNearest(octave * (TF_MAXOCTAVES-1))));
        ...
        for (eU32 j=0; j<unisono; j++)
        {
            ...
            while(len--)
            {
								eU32 off1 = static_cast<eU32>(eFtoL(*phase1 * (TF_IFFT_FRAMESIZE-1)) * 2);
								eU32 off2 = static_cast<eU32>(eFtoL(*phase2 * (TF_IFFT_FRAMESIZE-1)) * 2);
...
void eTfNoiseReset(eTfNoise &state)
{
    ...
    rand.SeedRandomly();
		...
		state.offset1 = static_cast<eU32>(rand.NextInt(0, TF_NOISETABLESIZE/2));
		state.offset2 = static_cast<eU32>(rand.NextInt(0, TF_NOISETABLESIZE/2));
...
void eTfFilterProcess(eTfFilter &state, eTfFilter::Type type, eF32 **signal, eU32 frameSize)
{
    ...
    if (type == eTfFilter::FILTER_LP)
    {
        ...
        while (len--)
        {
						eF32x2 in = eSimdSet2(static_cast<eF32>(*signal1), static_cast<eF32>(*signal2));
...
eF32 eTfInstrumentProcess(eTfSynth &synth, eTfInstrument &instr, eF32 **outputs, eU32 frameSize)
{
    ...
    for(eU32 k=0;k<TF_MAXVOICES;k++)
    {
        ...
        if (voice.noteIsOn || voice.playing)
        {
            ...
            //  CALCULATE VELOCITY
            // -------------------------------------------------------------------------------
						eF32 velocity = static_cast<eF32>(voice.currentVelocity) / 128.0f;
            ...
            // -------------------------------------------------------------------------------
						eU32 semiTonesUp = static_cast<eU32>(eFtoL(instr.params[TF_PITCHWHEEL_UP] * 12.0f + 1.0f));
						eU32 semiTonesDown = static_cast<eU32>(eFtoL(instr.params[TF_PITCHWHEEL_DOWN] * 12.0f + 1.0f));
    ...
    if (instr.effectsInactiveTime < TF_EFFECT_SWITCHOFF_TIME)
    {
        for(eU32 i=0;i<TF_MAXEFFECTS;i++)
        {
            ...
            eF32 fxVal = instr.params[TF_EFFECT_1 + i];
						eU32 fxIndex = static_cast<eU32>(eFtoL(eRoundNearest(fxVal * (FX_COUNT-1))));
    ...
    if (eIsFloatZero(peak))
				instr.effectsInactiveTime += static_cast<eF32>(frameSize) / static_cast<eF32>(synth.sampleRate);
...
{
		eU32 poly = static_cast<eU32>(eFtoL(instr.params[TF_GEN_POLYPHONY] * (TF_MAXVOICES-1) + 1));
...
void eTfStepSequencerSetInstrument(eTfStepSequencer &seq, eU32 step, eU32 instrument)
{
    eASSERT(step < TF_MAX_STEPSEQUENCE_LEN);
		seq.instrument[step] = static_cast<eS32>(instrument);
}
...
{
    eASSERT(step < TF_MAX_STEPSEQUENCE_LEN);
		return static_cast<eU32>(seq.instrument[step]);
}
...
{
	(void) seq; (void) sampleFrames; (void) outputs;
	return 0.0f;
}
...
void eTfSynthInit(eTfSynth &synth)
{
    ...
    eF64 a = 6.875;    // a
		a *= static_cast<eF64>(TF_12TH_ROOT_OF_2);    // b
		a *= static_cast<eF64>(TF_12TH_ROOT_OF_2);    // bb
		a *= static_cast<eF64>(TF_12TH_ROOT_OF_2);    // c, frequency of midi note 0

    for (eU32 i = 0; i < TF_NUMFREQS; i++)    // 128 midi notes
    {
        synth.freqTable[i] = static_cast<eF32>(a);
				a *= static_cast<eF64>(TF_12TH_ROOT_OF_2);
    }
    ...
    eMemSet(synth.instr, 0, TF_MAX_INSTR * sizeof(ePtrSize));
